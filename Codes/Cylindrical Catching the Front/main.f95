...
integer*4,parameter      :: n=9000,k=9000 ! n: time k: space
...
integer*4                :: i,j,js,bc,l,y,ms,g,m  ! Индексы текущего узла
...
real*8,   dimension(n,k) :: Tit,T,r        ! Поля итерационных
                                           ! переменных U,V
...
! Блок расчета параметров сетки
  ...
  real*8 :: Qz, St,zetast,tetast, Qov,flux,width

  !open (55,file='timem0.01.txt')
  !write (55,91) 'tau'
  !91 format (1(2x,A16))

  !open (11,file='s(time)m0.01.txt')
  !write (11,91)'zeta'
  !90 format(1(2x,A16))

  !open (57,file='width.txt')
  !write (57,92) 'width'
  !92 format (1(2x,A16))
  open (56,file='temps0.1.txt')
  write (56,91) 'temp'
  91 format (1(2x,A16))
  ...
  ms=2 !1 for melting, 2 for solidification
  bc=1 !1 for fixed temperature, 2 for fixed flux
...
! Блок начальных данных и граничных условий
  ...
  do j=2,k-1
    if(ms==1) T(1,j)=1-location**2d0
    if(ms==2) T(1,j)=Qz/4*(1-location**2d0)+1
    location=location+dr
  end do
  T(:,k)=Tout
...
! Основной цикл расчета
  ...
  do i=2,n            ! Перебор по x
    ...
    do it =1,400
      ...
      If (js==(l)) then
         ...
         a=1d0/(dr+0.5d0*dr**2d0/r(i,js))
         b=1d0/(dr-0.5d0*dr**2d0/r(i,js))
         dts=(0.5d0*dr**2d0*(a+b)*(1d0-T(i-1,js))+dr/St)/(b*(T(i,js+1)-1d0)-a*(1d0-T(i,js-1))+Qz*0.5d0*dr**2d0*(a+b)+Qz*dr)
         if(y==1)then
            dts=dabs(dts)
         end if
         y=y+1
      ...
      do j=2,k-1       ! Перебор по y
        r(i,j)=r(i,j-1)+dr
        abcd(j,1)= 1d0/dr**2d0-1d0/r(i,j)/dr/2d0
        abcd(j,2)= -2d0/dr**2d0-1d0/dt
        abcd(j,3)= 1d0/dr**2d0+1d0/r(i,j)/2d0/dr
  ...
  if(bc==2)then
      ...
      do j=1,k
        if ((T(i,j)<=1d0)) m=1
        if ((T(i,j)>=1d0).and.(m>=1)) then
            width = width+dr
            m=2
        end if
        if (j==k) then
            write(57,95) width
            102 format (1(5x,E16.8))
            exit
        end if
      end do
